...
export class PipelineBuilder<TIn, TOut> implements IPipelineBuilder<TIn, TOut> {
  protected steps: any[] = []; // Using any temporarily until we implement all step types
  private context?: IPipelineContext<TIn, TOut>;
  private source?: TIn;
  private compiledSteps?: any[]; // Using any temporarily until we implement all step types
  ...
  private extractPipesFromStep(step: any): IPipe<TIn, TOut>[] {
    if (step instanceof SequentialStep || step instanceof ConditionalStep) {
      return [step.pipe];
    } else if (step instanceof ParallelStep) {
      return [...step.pipes]; // Return a copy to prevent external modification
    } else if (step instanceof SubPipelineStep) {
      return step.getPipes();
    }
    // For any other step type that might implement getPipes
    if (step && typeof step.getPipes === 'function') {
      return step.getPipes();
    }
    // If we can't extract pipes from the step, return an empty array
    return [];
  }
  ...
  private async executeInternal(cancellationToken?: AbortSignal): Promise<void> {
    ...
    for (const step of this.compiledSteps) {
      ...
      if (this.context.performanceMetrics?.isEnabled && this.context.performanceMetrics.pipeDurations) {
        ...
        this.context.performanceMetrics.pipeDurations[pipeName] = duration;
      }
    }
   ...
   */
  private extractStepName(step: any): string {
    if (step instanceof SequentialStep) {
      return step.pipe.constructor.name;
    } else if (step instanceof ConditionalStep) {
      return `${step.pipe.constructor.name}_Conditional`;
    } else if (step instanceof ParallelStep) {
      return `Parallel_${step.pipes.map(p => p.constructor.name).join('_')}`;
    } else if (step instanceof SubPipelineStep) {
      return 'SubPipeline';
    }
    return 'UnknownStep';
