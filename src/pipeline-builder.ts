import {IPipelineBuilder} from './contracts/pipeline-builder.interface';
...
import {SequentialStep} from './steps/sequential-step';
import {ConditionalStep} from './steps/conditional-step';
import {ParallelStep} from './steps/parallel-step';
import {SubPipelineStep} from './steps/sub-pipeline-step';
...
import {RetryPolicy} from './error-handling/retry-policy';
import {CircuitBreakerPolicy} from './error-handling/circuit-breaker-policy';
import {FallbackPolicy} from './error-handling/fallback-policy';
import {RetryWithBackoffStrategy} from './error-handling/retry-with-backoff-strategy';
import {CircuitBreakerStrategy} from './error-handling/circuit-breaker-strategy';
import {DefaultPipelineValidator} from './validation/default-pipeline-validator';
import {PerformanceMetricsModel} from './models/performance-metrics.model';
import {MemoryMetricsModel} from './models/memory-metrics.model';
import {getMemoryUsage} from './utils/memory-tracker';
    ...
    enablePerformanceMetrics(correlationId?: string): IPipelineBuilder<TIn, TOut> {
        if (this.context) {
            if (!this.context.performanceMetrics) {
                this.context.performanceMetrics = new PerformanceMetricsModel();
            }
    ...
    private async executeInternal(): Promise<void> {
        ...
        if (this.context.performanceMetrics.isEnabled) {
            ...
            if (!this.context.performanceMetrics.memoryMetrics) {
                this.context.performanceMetrics.memoryMetrics = new MemoryMetricsModel();
            }
