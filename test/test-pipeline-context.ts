import {IPipelineContext} from '../src';
import {PipelineErrorModel} from '../src';
import {PerformanceMetricsModel} from '../src';
...
export class TestPipelineContext<TIn, TOut> implements IPipelineContext<TIn, TOut> {
    ...
    output: TOut;
    errors: string[] = [];
    pipelineErrors: PipelineErrorModel[] = [];
    ...
    addPipelineError(pipeName: string, error: Error, attemptNumber?: number): void {
        const pipelineError = new PipelineErrorModel(
            error.message,
            error
        );
    ...
    getPerformanceMetricsSummary(): string {
        if (!this.performanceMetrics) {
            this.performanceMetrics = new PerformanceMetricsModel();
        }
